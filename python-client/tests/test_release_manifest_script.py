...
def test_asset_key_mapping() -> None:
    ...
    assert key("xrayvpn-linux-arm64") == "linux-arm64"
    assert key("xrayvpn-macos-arm64") == "macos-arm64"
    assert key("xrayvpn_client-0.4.1-py3-none-any.whl") == "wheel"
    assert key("xrayvpn_client-0.4.1.tar.gz") == "sdist"
...
def test_build_manifest_urls_and_hashes(tmp_path: Path) -> None:
    (tmp_path / "xrayvpn-windows-x64.exe").write_bytes(b"win")
    ...
    assert win["url"].endswith("/releases/download/v0.4.1_experimental/xrayvpn-windows-x64.exe")
