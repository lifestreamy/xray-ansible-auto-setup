...
Параметры не обязательны: клиент можно запустить вообще без аргументов — `xrayvpn deploy` интерактивно спросит режим исполнения и IP VPS, затем скрыто запросит пароль. Минимальный случай — только IP VPS.
